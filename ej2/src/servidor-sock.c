...
void *gestionar_cliente (void *arg) {
    ...
    if (recvMessage(sock_cliente, (char*)&longitud_req_net, sizeof(longitud_req_net)) == -1) {
    ...
    char *req_json = malloc(longitud_req + 1);
    if (req_json == NULL) {
        perror("[SERVIDOR] Error al reservar memoria para la petición");
        close(sock_cliente);
        return NULL;
    }
    if (recvMessage(sock_cliente, req_json, longitud_req) == -1) {
        perror("[SERVIDOR] Error al recibir JSON de petición");
        free(req_json);
        close(sock_cliente);
        return NULL;
    }
    req_json[longitud_req] = '\0';
    ...
    if (sendMessage(sock_cliente, (char*)&longitud_res_net, sizeof(longitud_res_net)) == -1 ||
        sendMessage(sock_cliente, res_json, longitud_res) == -1) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        *sock_cliente = accept(sock_servidor, (struct sockaddr *)&addr_cliente, &len_cliente);
        ...
        // Hilo DETACHED: libera sus recursos automáticamente al terminar
        pthread_t hilo;
        pthread_attr_t attr;
