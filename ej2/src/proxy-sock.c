...
char* crear_json (struct peticion req){ 
    ...
    if (req.op == OP_INIT){ // EL tipo de operacion es destroy
        // Relleno el JSON
        cJSON_AddNumberToObject(obj, "op", OP_INIT);
    }

    else if (req.op == OP_SET){ // La operacion es set_value
        // Relleno el JSON
    ...
    else if (req.op == OP_GET){ // La operacion es get_value
        cJSON_AddNumberToObject(obj, "op", OP_GET);
        cJSON_AddStringToObject(obj, "key", req.key);
    }

    else if (req.op == OP_MODIFY){ // La operacion es modify_value
        // Relleno el JSON
    ...
    else if (req.op == OP_DELETE){ // La operacion es delete_key
        cJSON_AddNumberToObject(obj, "op", OP_DELETE);
        cJSON_AddStringToObject(obj, "key", req.key);
    }

    else if (req.op == OP_EXIST){ // La operacon es exist
...
struct respuesta traducir_json_response (char* json_response){ // Falta tratar errores
    ...
    respuesta_servidor.resultado = -1; // Valor por defecto por en caso de error

    // 1. Parseo el JSON con cJSON
    cJSON *obj = cJSON_Parse(json_response);
    if (obj == NULL) {
            perror("Error al parsear el JSON de la respuesta");
    ...
    if (result == NULL) {
        perror("Error: campo 'result' no encontrado en la respuesta");
        cJSON_Delete(obj);
        return respuesta_servidor;
    }
    respuesta_servidor.resultado = result->valueint;
...
int send_recv(char* request, char* response) {
    ...
    // 2. Resolver el host con gethostbyname
    struct hostent *hp; 
    struct sockaddr_in server_addr;
    hp = gethostbyname(ip_servidor);
    if (hp == NULL) {
        perror("Error en gethostbyname");
        return -2;
    }

    // 2.1 Relleno la estructura de direccion
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET; // IPV4
    server_addr.sin_port = htons(atoi(puerto_servidor)); // Puerto
    ...
    // 5.3 Compruebo que la respones cabe en el buffer
    if (longitud_response_final >= MAX_RESPONSE_SIZE) {
        perror("Error: la response es demasiado grande para el buffer");
        close(sock);
        return -2;
    }
    ...
    response[longitud_response_final] = '\0'; // Para que al parsear cJSON no falle
